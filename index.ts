...
import { normalizeStoragePath, type MemoryEntry, type MemoryStore } from "./src/store.js";
...
import { resolveScopeFilter, isSystemBypassId, parseAgentIdFromSessionKey } from "./src/scopes.js";
import {
  createMemoryCore,
  createMemoryExtractionRuntime,
  type MemoryCore,
} from "./src/core/memory-core.js";
...
import { createExtractionRateLimiter, stripEnvelopeMetadata, type SmartExtractor } from "./src/smart-extractor.js";
...
import { registerRemoteOpenClawMemory } from "./src/openclaw-remote.js";
import {
...
// Guard against duplicate hook/tool registration for a new API instance.
...
interface PluginSingletonState {
  ...
  vectorDim: number;
  store: MemoryStore;
  embedder: MemoryCore["embedder"];
  decayEngine: MemoryCore["decayEngine"];
  tierManager: MemoryCore["tierManager"];
  retriever: MemoryCore["retriever"];
  canonicalCorpusIndexer: CanonicalCorpusIndexer;
  dreamingEngine: DreamingEngine;
  dreamingScheduler: DreamingSchedulerState;
  scopeManager: MemoryCore["scopeManager"];
...
function _initPluginState(api: OpenClawPluginApi): PluginSingletonState {
  ...
  const resolvedDbPath = normalizeStoragePath(api.resolvePath(config.dbPath || getDefaultDbPath()));
  ...
  if (config.smartExtraction !== false || config.admissionControl?.enabled === true) {
    try {
      ...
      } as AdmissionControlConfig;
      const admissionExtractionLlm = admissionClientFor(
        admissionModelExtraction,
        globalThinkLevel,
        admissionModelExplicitExtraction,
      );
      // Keep the original sharing semantics: matching lane model + think level
      // reuses one controller/client even when modelExplicit provenance differs.
      const admissionReflectionLlm =
        admissionModelReflection === admissionModelExtraction && reflectionThinkLevel === globalThinkLevel
          ? admissionExtractionLlm
          : admissionClientFor(
              admissionModelReflection,
              reflectionThinkLevel,
              admissionModelExplicitReflection,
            );
      const extractionRuntime = createMemoryExtractionRuntime(core, {
        enabled: config.smartExtraction !== false,
        llmClient,
        admission: {
          config: admissionConfigWithChunk,
          extractionLlmClient: admissionExtractionLlm,
          reflectionLlmClient: admissionReflectionLlm,
          debugLog: (message) => api.logger.debug(message),
        },
        extractor: {
          user: "User",
      ...
      smartExtractor = extractionRuntime.smartExtractor;
      admissionController = extractionRuntime.admissionController;
      admissionControllerReflectionLane = extractionRuntime.reflectionAdmissionController;
...
const memoryLanceDBProPlugin = {
  ...
  register(api: OpenClawPluginApi) {
    ...
    const memoryConnection = parseMemoryConnectionConfig(api.pluginConfig);
    if (memoryConnection.mode === "remote") {
      _registeredApis.add(api);
      _registeredApisMap.set(api, true);
      try {
        const remote = memoryConnection.remote;
        const token = remote.token
          ? resolveSecretCredential(api, remote.token, "memory.remote.token")
          : undefined;
        registerRemoteOpenClawMemory(api, {
          config: {
            url: remote.url,
            token,
            ...
            scope: remote.scope,
            autoRecall: remote.autoRecall,
            autoCapture: remote.autoCapture,
            captureAssistant: remote.captureAssistant,
            autoRecallMinLength: remote.autoRecallMinLength,
            autoRecallMinRepeated: remote.autoRecallMinRepeated,
            autoRecallMaxItems: remote.autoRecallMaxItems,
            autoRecallMaxChars: remote.autoRecallMaxChars,
            autoRecallPerItemMaxChars: remote.autoRecallPerItemMaxChars,
            autoRecallMaxQueryLength: remote.autoRecallMaxQueryLength,
            maxCaptureChars: remote.maxCaptureChars,
          },
          onStop: () => {
        ...
        api.logger.info?.(
          `memory-lancedb-pro@${pluginVersion}: plugin registered in remote mode (server: ${remote.url})`,
        );
        return;
      } catch (err) {
        _registeredApis.delete(api);
        _registeredApisMap.delete(api);
        api.logger.error(`memory-lancedb-pro: remote mode initialization failed — ${String(err)}`);
        throw err;
      }
    }
...
export type MemoryConnectionConfig =
  | { mode: "embedded" }
  | {
      mode: "remote";
      remote: {
...
/**
 * Parse only the transport selection needed to decide whether OpenClaw should
 * initialize the local LanceDB stack. Remote mode intentionally does not call
 * parsePluginConfig(), so it can run without embedding or database settings.
 */
export function parseMemoryConnectionConfig(value: unknown): MemoryConnectionConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return { mode: "embedded" };
  }
  const initial = value as Record<string, unknown>;
  const nested = initial.config && typeof initial.config === "object" && !Array.isArray(initial.config)
    ? initial.config as Record<string, unknown>
    : undefined;
  const cfg = !initial.memory && !initial.embedding && nested ? nested : initial;
  const memory = cfg.memory;
  if (memory === undefined || memory === null) return { mode: "embedded" };
  if (typeof memory !== "object" || Array.isArray(memory)) {
    throw new Error("memory-lancedb-pro: config.memory must be an object");
  }
  const memoryRaw = memory as Record<string, unknown>;
  const mode = memoryRaw.mode ?? "embedded";
  if (mode !== "embedded" && mode !== "remote") {
    throw new Error("memory-lancedb-pro: memory.mode must be either 'embedded' or 'remote'");
  }
  if (mode === "embedded") return { mode: "embedded" };

  const remoteValue = memoryRaw.remote;
  if (remoteValue !== undefined && (typeof remoteValue !== "object" || remoteValue === null || Array.isArray(remoteValue))) {
  ...
  const url = asNonEmptyString(remoteRaw.url) ?? "http://127.0.0.1:7337";
  try {
    const parsed = new URL(resolveEnvVars(url));
    if (parsed.protocol !== "http:" && parsed.protocol !== "https:") throw new Error("unsupported protocol");
  } catch {
    throw new Error(`memory-lancedb-pro: memory.remote.url must be a valid http(s) URL: ${url}`);
  }
  ...
  return {
    ...
    remote: {
      ...
      token: token as SecretCredential | undefined,
