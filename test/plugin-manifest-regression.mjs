...
assert.deepEqual(
  manifest.configSchema.properties.memory.properties.mode.enum,
  ["embedded", "remote"],
  "memory.mode schema should expose embedded and remote modes",
);
assert.equal(
  manifest.configSchema.properties.memory.properties.mode.default,
  "embedded",
  "memory.mode should preserve embedded behavior by default",
);
assert.equal(
  manifest.configSchema.properties.memory.properties.remote.properties.url.default,
  "http://127.0.0.1:7337",
  "remote mode should default to the local standalone memory-server",
);
assert.ok(
  manifest.configSchema.properties.memory.properties.remote.properties.token.oneOf.some((entry) =>
    entry.type === "object" && entry.required?.includes("source") && entry.required?.includes("id")
  ),
  "memory.remote.token should accept OpenClaw SecretRef objects",
);
for (const hint of [
  "memory.mode",
  "memory.remote.url",
